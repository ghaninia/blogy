...
export default function NewExperiencePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await api.post<{ id: string }>('/api/experiences', formToExperiencePayload(form));
      toast({ title: tToast('saved'), variant: 'success' });
      router.push(`/${locale}/dashboard/experiences/${res.data?.id}/edit`);
    } catch (err) {
      toast({
        title: tToast('error'),
